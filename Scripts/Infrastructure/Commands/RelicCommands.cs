...
public class AddRelicCommand : DevConsoleCommand
{
    ...
    {
        if (args.Length < 1) return CommandResult.Fail($"用法: /addrelic <relic_id>  可用: {string.Join(", ", _relicDefs.Keys)}");
    ...
    private static void EnsureRelicCache()
    {
        ...
        foreach (var r in relics) _relicDefs[r.Id] = r;
    }
}
