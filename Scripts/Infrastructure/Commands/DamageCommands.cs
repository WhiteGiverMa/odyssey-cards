...
public class DamageESlotCommand : DevConsoleCommand
{
    ...
    public override CommandResult Execute(string[] args)
    {
        var cm = CombatManager.Instance;
        if (cm == null) return CommandResult.Fail("未在战斗中");
        if (args.Length < 2) return CommandResult.Fail("用法: /damage_eslot <槽位0-4> <伤害值>");
        if (!int.TryParse(args[0], out var slot) || slot < 0 || slot > 4) return CommandResult.Fail("槽位需为 0-4");
        int dmg = int.Parse(args[1]);
        var m = cm.Board.GetMinionAt(slot, isPlayerSide: false);
        if (m == null || m.IsDead) return CommandResult.Fail($"敌方槽位 {slot} 无有效随从");
        m.TakeDamage(dmg, null);
        string msg = $"对敌方槽位{slot} {m.CardName} 造成 {dmg} 点伤害（剩余 {m.CurrentHealth}）";
        if (m.IsDead) cm.Board.RemoveMinion(m);
        cm.CheckDeaths();
        cm.CheckVictoryOrDefeat();
        RefreshCombatUI(cm);
...
public class DamagePSlotCommand : DevConsoleCommand
{
    ...
    public override CommandResult Execute(string[] args)
    {
        var cm = CombatManager.Instance;
        if (cm == null) return CommandResult.Fail("未在战斗中");
        if (args.Length < 2) return CommandResult.Fail("用法: /damage_pslot <槽位0-4> <伤害值>");
        if (!int.TryParse(args[0], out var slot) || slot < 0 || slot > 4) return CommandResult.Fail("槽位需为 0-4");
        int dmg = int.Parse(args[1]);
        var m = cm.Board.GetMinionAt(slot, isPlayerSide: true);
        if (m == null || m.IsDead) return CommandResult.Fail($"己方槽位 {slot} 无有效随从");
        m.TakeDamage(dmg, null);
...
public class DamageAllCommand : DevConsoleCommand
{
    ...
    public override CommandResult Execute(string[] args)
    {
        ...
        foreach (var e in enemies) { e.TakeDamage(n, null); if (e.IsDead) cm.Board.RemoveMinion(e); }
        cm.CheckDeaths();
        cm.CheckVictoryOrDefeat();
        RefreshCombatUI(cm);
