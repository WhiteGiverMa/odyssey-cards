...
internal sealed class DomainTriggerManager
{
    ...
    {
        if (!ReferenceEquals(target, _playerHero)) return;
        ...
        if (!_playerHero.ActiveDomains.TryGetValue("flying_away", out var domain)) return;
        if (domain.LastTriggeredTurn == _state.TurnCount) return;
