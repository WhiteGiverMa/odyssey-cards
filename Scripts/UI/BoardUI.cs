...
public partial class BoardUI : Control
{
	...
	{
		if (!_keyboardTargetingEnabled || !IsInsideTree())
			return;
	...
	{
		// 目标行是否已启用
		if (toPlayerSide && !_keyboardTargetingPlayerSlots)
			return;
		if (!toPlayerSide && !_keyboardTargetingEnemySlots)
			return;
	...
	{
		if (isPlayerSide && !_keyboardTargetingPlayerSlots)
			return false;
		if (!isPlayerSide && !_keyboardTargetingEnemySlots)
			return false;
	...
	public partial class BoardSlot : Control
	{
		...
		public void UpdateDisplay(Minion? minion)
		{
			...
			if (minion.HasTaunt)
				keywords.Add(Localization.Localization.T("ui.board.keyword_taunt", "嘲"));
			if (minion.HasCharge)
				keywords.Add(Localization.Localization.T("ui.board.keyword_charge", "冲"));
			if (minion.HasWindfury)
				keywords.Add(Localization.Localization.T("ui.board.keyword_windfury", "风"));
			if (minion.HasBattlecry)
				keywords.Add(Localization.Localization.T("ui.board.keyword_battlecry", "吼"));
			if (minion.HasDeathrattle)
				keywords.Add(Localization.Localization.T("ui.board.keyword_deathrattle", "亡"));
			if (minion.HasAmbush)
				keywords.Add(Localization.Localization.T("ui.board.keyword_ambush", "伏"));
			if (minion.HasImpact)
				keywords.Add(Localization.Localization.T("ui.board.keyword_impact", "击"));
