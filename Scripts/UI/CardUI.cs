...
namespace OdysseyCards.UI
{
	public partial class CardUI : Control
	{
		...
		public event Action<CardUI, ICommander> OnCardDraggedToTarget;
		...
		private void ProcessDragEnd()
		{
			...
			ICommander targetChar = DetectCharacterTarget(globalMousePos);
			if (targetChar != null)
			{
		...
		private ICommander DetectCharacterTarget(Vector2 globalPos)
		{
			...
			foreach (Node node in (Godot.Collections.Array<Node>)GetTree().GetNodesInGroup("Ally"))
			{
				if (node is Control container)
				{
					...
					if (globalRect.HasPoint(globalPos))
					{
						...
						{
							Player targetChar = container.GetMeta("CharacterObject").As<Player>();
							if (targetChar != null && order.IsValidTarget(targetChar, player))
							{
								return targetChar;
							}
