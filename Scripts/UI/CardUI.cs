...
namespace OdysseyCards.UI
{
	public partial class CardUI : Control
	{
		...
		{
			GD.Print($"[CardUI] HandleLeftRelease: {Card?.CardName}, IsDragging: {IsDragging}, _isDragActive: {_isDragActive}");
			...
			foreach (Node node in (Godot.Collections.Array<Node>)GetTree().GetNodesInGroup("Enemy"))
			{
				if (node is Control container)
				{
					...
					if (globalRect.HasPoint(globalPos))
					{
						...
						{
							return container.GetMeta("EnemyObject").As<Character.Character>();
						}
					}
				}
			}
		...
		{
			foreach (Node node in (Godot.Collections.Array<Node>)GetTree().GetNodesInGroup("Headquarters"))
			{
