...
public partial class SettingsPage : Control
{
    ...
    private void OnEmoteIdleTimeChanged(double value)
    {
        ...
        var gm = GameManager.Instance;
        if (gm != null) gm.EmoteIdleTimeSeconds = v;
    }
    ...
    {
        var im = InputManager.Instance;
        if (im == null || index < 0 || index >= _profileSelector.ItemCount) return;
    ...
    private void OnNewProfile()
    {
        ...
        do { name = $"配置 {n}"; n++; } while (existing.Contains(name));
    ...
    private void OnDeleteProfile()
    {
        ...
        if (idx < 0 || idx >= _profileSelector.ItemCount) return;
    ...
    public override void _Input(InputEvent @event)
    {
        ...
        if (@event is not InputEventScreenTouch touch || !touch.Pressed) return;
    ...
    {
        if (control == null || !control.IsInsideTree() || !control.Visible) return false;
    ...
    {
        if (optionButton.ItemCount <= 0) return;
