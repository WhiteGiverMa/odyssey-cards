...
func _resolve_supported_binding_from_action_mapping(action_mapping) -> Dictionary:
	for input_mapping in action_mapping.input_mappings:
		...
		if input.get_class() == "GUIDEInputKey":
			return {
				"kind": "key",
		...
		if input.get_class() == "GUIDEInputMouseButton":
			return {
				"kind": "mouse_button",
				"button_index": int(input.button),
			}
		if input.get_class() == "GUIDEInputJoyButton":
			return {
				"kind": "joy_button",
				"button_index": int(input.button),
...
func _cmd_ui_theme(params: Dictionary) -> void:
	...
	# Font size overrides
	var font_sizes: Dictionary = overrides.get("fontSizes", {})
