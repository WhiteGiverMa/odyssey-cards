...
/// <summary>
/// 单元测试 — MinionDeployVfx.QuadraticBezier 二次贝塞尔插值数学。
/// 纯数学函数，无需 Godot 运行时。
/// </summary>
public class MinionDeployVfxTests
{
	[Fact]
	public void QuadraticBezier_Start_ReturnsStart()
	{
		...
		var result = MinionDeployVfx.QuadraticBezier(0f, start, ctrl, end);
	...
	public void QuadraticBezier_End_ReturnsEnd()
	{
		...
		var result = MinionDeployVfx.QuadraticBezier(1f, start, ctrl, end);
	...
	public void QuadraticBezier_Halfway_ReturnsMidpoint()
	{
		...
		var result = MinionDeployVfx.QuadraticBezier(0.5f, start, ctrl, end);
	...
	public void QuadraticBezier_Quarter_ReturnsCorrectValue()
	{
		...
		var result = MinionDeployVfx.QuadraticBezier(0.25f, start, ctrl, end);
	...
	public void QuadraticBezier_ThreeQuarter_ReturnsCorrectValue()
	{
		...
		var result = MinionDeployVfx.QuadraticBezier(0.75f, start, ctrl, end);
	...
	[Fact]
	public void CalculateFitRenderScale_StandardBoardSlot_FitsByHeight()
	{
		var targetSize = new Vector2(100f, 140f);

		float result = MinionDeployVfx.CalculateFitRenderScale(targetSize);

		Assert.Equal(140f / CardUI.DESIGN_HEIGHT, result, 5);
		Assert.True(CardUI.DESIGN_WIDTH * result <= targetSize.X);
		Assert.True(CardUI.DESIGN_HEIGHT * result <= targetSize.Y);
	}

	[Fact]
	public void CalculateFitRenderScale_WideSlot_FitsByHeight()
	{
		var targetSize = new Vector2(300f, 90f);

		float result = MinionDeployVfx.CalculateFitRenderScale(targetSize);

		Assert.Equal(90f / CardUI.DESIGN_HEIGHT, result, 5);
	}

	[Fact]
	public void CalculateFitRenderScale_TallNarrowSlot_FitsByWidth()
	{
		var targetSize = new Vector2(60f, 300f);

		float result = MinionDeployVfx.CalculateFitRenderScale(targetSize);

		Assert.Equal(60f / CardUI.DESIGN_WIDTH, result, 5);
	}
}
