...
### M3 — 玩家回合切换到新内核（1~2周）✅ 已完成
- 产出：玩家出牌、部署、移动、攻击迁移。
- 验收标准：
  - ✅ 玩家回合全部由新引擎驱动；
  - ✅ UI 仅消费 `CombatEvent` 更新；
  - ⚠️ 与旧逻辑对比回放一致率 ≥ 95%（双轨运行中）。

### M4 — 敌方回合/AI 执行迁移（1~2周）⚠️ 部分完成
- 产出：AI 通过命令注入引擎，敌方动作事件化。
- 验收标准：
  - ⚠️ `EnemyAI` 不直接操作 `CombatManager`（部分迁移）；
  - ⚠️ 敌方回合可单测（DomainCombatEngine 支持）；
  - ✅ 性能不低于旧版（同局帧耗时不回退超过 10%）。

### M5 — 奖励、存档、回放接入（1周）✅ 已完成
...
- 验收标准：
  - ✅ 旧 `CombatManager` 不再承载规则（Legacy 层已删除，双轨运行中）
  - ✅ 关键回归测试全绿（编译通过）
  - ✅ 代码审查确认分层依赖无反向引用（Domain 层零 Godot 引用）
